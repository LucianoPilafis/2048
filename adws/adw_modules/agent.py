...
def prompt_claude_code(request: AgentPromptRequest) -> AgentPromptResponse:
    ...
    try:
        ...
        with open(request.output_file, "w") as f:
            result = subprocess.run(
                cmd, stdout=f, stderr=subprocess.PIPE, text=True, env=env, timeout=300
            )

        # Check if output file was created
        if not os.path.exists(request.output_file):
            error_msg = f"Error: Claude Code did not produce output file"
        ...
        error_msg = "Error: Claude Code command timed out after 5 minutes. The command may be waiting for interactive input or the API may not be responding."
        print(error_msg, file=sys.stderr)
        return AgentPromptResponse(output=error_msg, success=False, session_id=None)
    except Exception as e:
